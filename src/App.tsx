...
const App = ({ loading, loadingText, userInfo }: Props) => {
  ...
  React.useEffect(() => {
    if (userInfo.id && materMarker.current) {
      addWaterMarker(userInfo.nickname + '    ' + userInfo.username, materMarker.current);
    }
  }, [userInfo.id]);
  return (
    <>
      <div ref={materMarker} style={{ position: 'fixed', top: 0, left: 0, bottom: 0, right: 0, zIndex: 999 }}></div>
